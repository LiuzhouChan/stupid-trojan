...
bool CSmtp::CreateConn()
{
	...
	hints.ai_protocol = IPPORT_SMTP; 
	const char *addr;
	struct sockaddr_in *sinp;
	getaddrinfo(domain.c_str(),"25", &hints, &result);
	sinp = (struct sockaddr_in *)result->ai_addr;
	char buf[INET_ADDRSTRLEN];
	addr = inet_ntop(AF_INET, &sinp->sin_addr, buf, INET_ADDRSTRLEN);
	//得到smtp服务器的网络字节序的ip地址  
	addrSrv.sin_addr.S_un.S_addr = *((DWORD *)addr);

	cout << addrSrv.sin_addr.S_un.S_addr<<endl;
	cout << addr << endl;
